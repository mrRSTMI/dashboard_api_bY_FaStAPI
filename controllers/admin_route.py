...
from services.admin_service import get_admin_service_by_id,get_admin_service, new_admin_service,update_admin_service,delete_admin_service
router = APIRouter(
    prefix="/api/admin",
    tags=["Admin"]
)
...
async def update_admin_by_id(ID: int, update_admin : UpdateAdmin,db:Session = Depends(get_db)):
